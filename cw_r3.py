...
DEBUG_MODE = False      # if false: only CRITICAL-, ERROR- and WARNING- level events will be logged
...
class SimpleBot:
    def __init__(self, player, opponent):
        ...
        self.card_priority = [
            "Curse", "Sorcerer", "School", "Recruit", "Conjure Crystals", "Thief",
            "Conjure Weapons", "Conjure Bricks", "Crush Crystals", "SWAT", "Wain",
            "Reserve", "Fence", "Fort", "Banshee", "Pixies", "Crush Bricks",
            "Crush Weapons", "Defence", "Tower", "Dragon", "Babylon", "Saboteur",
            "Attack", "Wall", "Base", "Platoon", "Rider", "Archer", "Knight"
        ]
        self.generator_cards = ["Sorcerer", "School", "Recruit"]
    ...
    def choose_action(self):
        ...
        if generator_in_hand and not self.is_under_threat():
        ...
        if playable_cards:
            try:
                ...
                sorcerer_index = next((i for i in playable_cards if self.player.cards[i].name == "Sorcerer"), None)
                if sorcerer_index is not None and not self.is_under_threat():
                ...
                logging.error(f"Error occurred while evaluating card: {self.player.cards[playable_cards[0]].name}")
                logging.error(f"Card effect: {self.player.cards[playable_cards[0]].effect}")
        ...
        for effect in effects:
            ...
            try:
                if "castle +" in effect:
                    ...
                    benefit += 15
                elif "enemy" in effect and "stocks" not in effect:  # Handling cases like "enemy castle -10"
                    benefit += int(effect.split()[-1]) * 1.5
                elif "stocks" in effect:  # Handling cases like "enemy stocks -4" or "transfer enemy stocks 5"
                    benefit += 10  # Assign a fixed benefit for stock manipulation

            except ValueError as e:
                logging.warning(f"Could not evaluate benefit for effect: {effect}. Error: {str(e)}")
                benefit += 5  # Assign a default benefit if we can't parse the effect

        return benefit


    def choose_discard(self):
        # Sort cards based on priority (reverse order, so least valuable cards are first)
        sorted_cards = sorted(enumerate(self.player.cards), 
                              key=lambda x: self.card_priority.index(x[1].name) if x[1].name in self.card_priority else -1,
                              reverse=True)
        
        logging.debug(f"Sorted cards for discard: {[card.name for _, card in sorted_cards]}")
        
        # Choose the least valuable card to discard
        discard_index, discard_card = sorted_cards[0]
        
        logging.debug(f"Bot is discarding card: {discard_card.name}")
